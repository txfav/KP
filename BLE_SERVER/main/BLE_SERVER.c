...
void send_data_firebase () {
    ...
    cJSON_AddNumberToObject(patch_json, "Pintu/", nsPintu);
...
void send_thingspeak(float data1, float data2){
    ...
    char* post_data = (char*) malloc(sizeof(char) * 50);
    sprintf(post_data, "api_key=EOTNWO4C76VB6OQF&field1=%.1f&field2=%.1f", data1, data2);
...
void dht_sensor(void*pvParameters){
    ...
    float humidity;
    while(1){
        if (dht_read_float_data(DHT_TYPE_AM2301,pinDHT, &humidity, &temperature) == ESP_OK){
            send_thingspeak(temperature, humidity);
        }else{
...
static const struct ble_gatt_svc_def gatt_svcs[] = {
    {
        ...
        .uuid = BLE_UUID128_DECLARE(GATT_SPP_SVC_UUID),
        .characteristics = (struct ble_gatt_chr_def[]) { {
            .uuid = BLE_UUID128_DECLARE(GATT_SPP_CHR_UUID),
...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    if (event_id == WIFI_EVENT_STA_START) {
        ...
        xTaskCreate(send_data, "Send_data", 3072,NULL,3,NULL);
        //xTaskCreate(dht_sensor, "Baca dht dan Kirim data", 4096, NULL, 3, NULL);
    }
}
